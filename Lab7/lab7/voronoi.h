...
void voronoi()
{
	...
	{
		p.x = features[i].getEnvelope().getMinX();
		p.y = features[i].getEnvelope().getMinY();
		points.push(p);
	...
	double dx = (X1 - X0) / 5.0; double dy = (Y1 - Y0) / 5.0;
